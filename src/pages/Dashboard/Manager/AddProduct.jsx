...
import { FiType, FiFileText, FiDollarSign, FiHash, FiVideo, FiGrid, FiUploadCloud, FiPlus, FiX } from 'react-icons/fi';
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div>
                        ...
                        <div className="flex items-center gap-3 border border-base-300 rounded-xl px-4 py-3">
                            <FiType /><input type="text" placeholder="Product Title" className="bg-transparent outline-none w-full" {...register('title', { required: true })} />
                        </div>
                    </div>
                    <div>
                        ...
                        <div className="flex items-start gap-3 border border-base-300 rounded-xl px-4 py-3">
                            <FiFileText className="mt-1" /><textarea placeholder="Description" className="bg-transparent outline-none w-full" rows={3} {...register('description', { required: true })} />
                        </div>
                    </div>
                    <div className="grid md:grid-cols-2 gap-5">
                        ...
                        <div className="flex items-center gap-4">
                            <label className="flex items-center gap-2"><input type="radio" value="cod" className="radio radio-primary" defaultChecked {...register('paymentOption')} /><span>COD</span></label>
                    ...
                    <div className="grid md:grid-cols-3 gap-5">
                        <input type="number" step="0.01" placeholder="Price" className="input input-bordered" {...register('price', { required: true })} />
                        <input type="number" placeholder="Quantity" className="input input-bordered" {...register('quantity', { required: true })} />
                        <input type="number" placeholder="MOQ" className="input input-bordered" {...register('moq')} />
                    </div>
                    <div>
                        ...
                        <label className="flex flex-col items-center gap-2 border-2 border-dashed border-base-300 rounded-xl p-6 cursor-pointer hover:border-primary/50 transition-colors">
                            ...
                            <span className="text-sm text-base-content/50">Click to select images</span>
