...
const AdminAllOrders = () => {
    ...
    const { data: orders = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            return (await axiosSecure.get(`/orders/all?${params.toString()}`)).data;
        }
    });
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
                <label className="input input-bordered input-sm flex items-center gap-2 flex-1 min-w-[200px]"><FiSearch /><input type="text" placeholder="Search orders..." className="grow" value={search} onChange={e => setSearch(e.target.value)} /></label>
                ...
                <div className="overflow-x-auto bg-base-100 rounded-xl border border-base-300">
                    <table className="table table-zebra w-full">
                        ...
                        <tbody>
                            {orders.map((o, i) => (
                                <tr key={o._id}>
                                    <td>{i + 1}</td><td className="font-medium">{o.productTitle}</td><td className="text-xs">{o.userEmail}</td><td className="font-bold">${o.totalPrice}</td><td>{statusBadge(o.status)}</td>
                                    <td><span className={`badge badge-sm ${o.paymentStatus === 'paid' ? 'badge-success' : 'badge-warning'}`}>{o.paymentStatus || 'cod'}</span></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
