        ...
        <div className="max-w-md mx-auto px-4 py-16 text-center">
            <div className="card bg-base-100 border border-base-300 shadow-xl rounded-2xl overflow-hidden">
                <div className="bg-primary/10 p-6 flex justify-center">
                    <div className="w-16 h-16 bg-primary/20 rounded-full flex items-center justify-center text-primary text-3xl"><FiCreditCard /></div>
                </div>
                <div className="card-body p-8">
                    <h2 className="text-2xl font-bold mb-2">Complete Payment</h2>
                    <p className="text-base-content/60 mb-6 font-medium">You are about to pay for your order via Stripe securely.</p>
                    <div className="bg-base-200/50 rounded-xl p-4 mb-6 text-left space-y-3">
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-base-content/60 flex items-center gap-2"><FiPackage /> Product</span>
                            <span className="font-semibold truncate max-w-[150px]">{order.productTitle}</span>
                        </div>
                    ...
                    <button onClick={handlePayment} disabled={processing} className="btn btn-primary w-full h-12 text-base">
                        {processing ? 'Redirecting…' : 'Pay with Stripe'}
                    </button>
                    <p className="text-xs text-base-content/40 mt-4">Safe and secure payments powered by Stripe.</p>
                </div>
            </div>
        </div>
    );
